...
class MLP(torch.nn.Module):
    def __init__(self, num_mlp_layers = 5, emb_dim = 300, drop_ratio = 0):
        ...
        for i in range(self.num_mlp_layers - 2):
            module_list += [torch.nn.Linear(self.emb_dim, self.emb_dim),
            torch.nn.BatchNorm1d(self.emb_dim),
            torch.nn.ReLU(),
            torch.nn.Dropout(p = self.drop_ratio)]
        
        module_list += [torch.nn.Linear(self.emb_dim, 256),
...
#         module_list += [torch.nn.Linear(self.emb_dim, 1)]
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            return x,torch.clamp(output, min=0, max=50)
...
dataset = PCQM4MDataset(root='~/lsc/pcqm4m/dataset', only_smiles=True)
...
for epoch in range(1, 51):
    ...
    if valid_mae < best_valid_mae:
        ...
        folder = os.getcwd()
        torch.save(model.state_dict(),folder+'/'+'model_mlp.pt')
        if args.checkpoint_dir is not '':
...
# import os
# folder = os.getcwd()
# model = MLP(num_mlp_layers=6, emb_dim=1600, drop_ratio=0.2).to(device)
# model.load_state_dict(torch.load(folder+'/'+'model_mlp.pt'))
# forest_pretrain_data = []
# # dataset = PCQM4MDataset(root='dataset/', only_smiles=True)
# # fp_processed_file = preprocess_fp(dataset, args.radius)
# fp_processed_file = '/home/wch/KDD_CUP/examples/lsc/pcqm4m/dataset/pcqm4m_kddcup2021/fp_processed/data_radius2.pt'
# data_dict = torch.load(fp_processed_file)
# X, Y = data_dict['X'], data_dict['Y']
# import pandas as pd
# forest_pretrain_data = []
...
#     x = X[i].to(device).view(1,-1).to(torch.float32)
#     x,pred = model(x)
#     forest_pretrain_data.append(x.tolist())
# y = Y[:1000000]
# df_label = pd.DataFrame(y)
# df_label.to_csv(folder+'/forest_target.csv')
